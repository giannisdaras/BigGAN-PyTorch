...
def load_inception_net(parallel=False):
  ...
  inception_model = WrapInception(inception_model.eval()).to(xm.xla_device())
  if parallel:
...
def prepare_inception_metrics(dataset, parallel, no_fid=False):
  ...
  def get_inception_metrics(sample, num_inception_images, num_splits=10,
    ...
    if no_fid:
      ...
      if use_torch:
        FID = torch_calculate_frechet_distance(mu, sigma, torch.tensor(data_mu).float().to(xm.xla_device()), torch.tensor(data_sigma).float().to(xm.xla_device()))
